...
from carla.controller.utils import *
...
STOP_SIGN_FILE        = 'stop_sign_params.txt'
STOP_SIGN_FENCELENGTH = 5        # m
PARKED_CAR_FILE       = 'parked_vehicle_params.txt'
...
def follow_waypoints(args):
    ...
    with make_carla_client(args.host, args.port) as client:
        ...
        with open(STOP_SIGN_FILE, 'r') as stopsign_file:
        ...
        with open(PARKED_CAR_FILE, 'r') as parkedcar_file:
        ...
        lp = local_planner.LocalPlanner(NUM_PATHS,
                                        ...
                                        SLOW_SPEED,
                                        STOP_LINE_BUFFER)
        bp = behavioral_planner.BehavioralPlanner(BP_LOOKAHEAD_BASE,
                                                    stopsign_fences,
        ...
        for frame in range(TOTAL_EPISODE_FRAMES):
            ...
            if frame % LP_FREQUENCY_DIVISOR == 0:
                ...
                bp.set_lookahead_distance(BP_LOOKAHEAD_BASE + BP_LOOKAHEAD_TIME * open_loop_speed)

                # Perform a state transition in the behavioural planner.
                bp.state_transition(waypoints, ego_state, current_speed, lead_car_pos[1])

                # Compute the goal state set from the behavioural planner's computed goal state.
                goal_state_set = lp.get_goal_state_set(bp.goal_index, bp.goal_waypoint, waypoints, ego_state)

                # Calculate planned paths in the local frame.
                paths, path_validity = lp.plan_paths(goal_state_set)

                # Transform those paths back to the global frame.
                paths = local_planner.LocalPlanner.transform_paths(paths, ego_state)

                # Perform collision checking.
                collision_check_array = lp.collision_checker.collision_check(paths, [parkedcar_box_pts])

                # Compute the best local path.
                best_index = lp.collision_checker.select_best_path_index(paths, collision_check_array, bp.goal_waypoint)
                ...
                desired_speed = bp.goal_waypoint[2]
                lead_car_state = [lead_car_pos[1][0], lead_car_pos[1][1], lead_car_speed[1]]
                decelerate_to_stop = bp.fsm_state == behavioral_planner.DECELERATE_TO_STOP
                local_waypoints = lp.velocity_planner.compute_velocity_profile(best_path,
                                                                               desired_speed,
                                                                               ego_state,
                                                                               current_speed,
                                                                               decelerate_to_stop,
                                                                               lead_car_state,
                                                                               bp.follow_lead_vehicle)
                # --------------------------------------------------------------
